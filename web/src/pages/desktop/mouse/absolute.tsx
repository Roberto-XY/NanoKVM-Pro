...
import { registerMouseReleaseHandler, sendMouseRelease, sendMouseReport } from './lifecycle.ts';
...
export const Absolute = () => {
  ...
  // Pinch baselines, re-captured on every pinch, so they need no lifecycle reset.
  const pinchStartDistRef = useRef(0);
  const pinchStartScaleRef = useRef(1);
  const pinchStartMidRef = useRef({ x: 0, y: 0 });
  const pinchStartPanRef = useRef({ x: 0, y: 0 });

  // Mirror zoom/pan into a ref so pinch handlers read live values without
  ...
  useEffect(() => {
    ...
    function handleTouchStart(e: TouchEvent) {
      ...
      if (e.touches.length > 1) {
        // Entering multi-touch ends any single-finger drag/long-press before zooming.
        releaseMouse({ cancelTouchSequence: false });
    ...
    function handleTouchMove(e: TouchEvent) {
      ...
      if (isMultiTouchRef.current) {
        ...
        if (startDist <= 0) {
          // Both fingers landed on the same point; no baseline to scale against.
          return;
        }
        ...
        const scale = Math.max(
          MIN_SCALE,
          Math.min(MAX_SCALE, startScale * (currentDist / startDist))
        );

        // Anchor the gesture: the content point under the initial midpoint stays
        // under the current midpoint. The transform origin is the element centre.
        ...
        const panX =
          currentMidX -
          halfW -
          (pinchStartMidRef.current.x - halfW - pinchStartPanRef.current.x) * scaleRatio;
        const panY =
          currentMidY -
          halfH -
          (pinchStartMidRef.current.y - halfH - pinchStartPanRef.current.y) * scaleRatio;

        setVideoParameters((prev) => ({ ...prev, scale, panX, panY }));
        return;
      }
